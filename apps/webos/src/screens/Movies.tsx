...
import { fetchMovies, isAbortError } from '../api'
import {
  CATALOG_LIMIT,
  chunkIntoRails,
  fetchCatalogPages,
...
export function MoviesScreen({ genre, onNavigate }: MoviesScreenProps) {
  const [rails, setRails] = useState<CatalogRail[]>([])
  const [error, setError] = useState<string | null>(null)
  const [loading, setLoading] = useState(true)
  const [reloadKey, setReloadKey] = useState(0)
  const rk = routeKey({ name: 'movies', genre })
  ...
  useEffect(() => {
    ...
    const load = genre
      ? fetchCatalogPages(
          (page, signal) => fetchMovies({ genre, page }, signal),
          ac.signal,
          CATALOG_LIMIT,
        )
      : fetchCatalogPages(
          (page, signal) => fetchMovies({ category: 'popular', page }, signal),
          ac.signal,
          CATALOG_LIMIT,
        )
    ...
    load
      .then((items) => {
        ...
        setRails(
          chunkIntoRails(
            capped,
            genre ? ['In This Genre', 'More In Genre', 'Even More'] : MOVIE_RAIL_LABELS,
          ),
        )
        setLoading(false)
      })
  ...
  const open = (item: MediaSummary, focusId: string) => {
    ...
    onNavigate({ name: 'movie', id: item.id })
  }
  ...
  return (
    <div className="screen screen--catalog">
      <header className="catalog-header">
        <p className="catalog-header__eyebrow">Movies</p>
        <h1>{genre ? 'Genre Collection' : 'Movies'}</h1>
        ...
        <div className="catalog-rails">
          {rails.map((rail, index) => (
            <MediaRow
              ...
              onSelect={(item) => open(item, `movies-${rail.id}-${item.type}-${item.id}`)}
